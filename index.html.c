...
<!DOCTYPE html>
  ...
  function sendtankdata(){
    ...
    // ppm: [LX, LY, RX, RY, BTNS]
    LX = ppm[0]; // Left joystick X
    LY = ppm[1]; // Left joystick Y
    RX = ppm[2]; // Right joystick X
    RY = ppm[3]; // Right joystick Y
    BTNS = ppm[4];  // Buttons
    ...
    sendframe[0] = 255 * Math.max(0, Math.min(1.0,(v + 1)/2)); // longitudinal velocity
    sendframe[1] = 255 * Math.max(0, Math.min(1.0,(omega + 1)/2)); // angular velocity
    sendframe[2] = BTNS;
    if (wsconnect) connection.send(sendframe);
    
    /* 
     * Legacy magic happened after this
    */
    //var LY;
    //var RX;
    //if(ppm[4] & 0x01){
